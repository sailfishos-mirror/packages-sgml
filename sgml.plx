...
White space is passed literally to the application. This mode leaves most
white space handling to the application. This is the default mode for
the XML dialect. Note that \verb$\r\n$ is still translated to \verb$\n$.
To preserve whitespace \emph{exactly}, use \term{space}{strict} (see
below)

    \termitem{space}{strict}
White space is passed strictly to the application. This mode leaves all
white space handling to the application. This is useful for producing
and verifying XML signatures.
...
The DTD \const{html} is handled separately. The Prolog flag
...
input embedded in \jargon{envelopes}, such as the HTTP protocol.
    \termitem{cdata}{+Representation}
Specify the representation of \jargon{cdata} elements. Supported are
\const{atom} (default), and \const{string}.  See load_structure/3 for
details.
    \termitem{parse}{+Unit}
...
predicates and acceptable by xml_write/3 and friends.
\end{description}
